...
fun HomeScreen(
    ...
    LaunchedEffect(Unit) {
        // Load M3U file on IO thread to avoid blocking UI
        viewModel.fetchM3UFile()
    }

    // Load movies from Vega providers on background thread - keyed on installedExtensionsState
            ...
            try {
                withContext(Dispatchers.Default) {
                    ...
                    if (firstScraper != null) {
                        ...
                        // Fetch catalog on IO thread
                        val (catalogs, _) = withContext(Dispatchers.IO) {
                            runner.getCatalog(provider.url, firstScraper.value)
                        }
                        val postsMap = mutableMapOf<VegaCatalog, List<VegaPost>>()

                        val catalogsToFetch = catalogs.take(3) // Reduced from 6 to 3 for faster initial load
                        if (catalogsToFetch.isEmpty()) {
                            val posts = withContext(Dispatchers.IO) {
                                runner.getPosts(provider.url, firstScraper.value, filter = "", page = 1)
                            }
                            if (posts.isNotEmpty()) {
                                moviesByCategory = mapOf(VegaCatalog("Featured", "") to posts.take(15))
                            }
                        } else {
                            // Fetch first category immediately, then others in parallel
                            val firstCat = catalogsToFetch.first()
                            val firstPosts = withContext(Dispatchers.IO) {
                                runner.getPosts(provider.url, firstScraper.value, filter = firstCat.filter, page = 1)
                            }
                            if (firstPosts.isNotEmpty()) {
                                postsMap[firstCat] = firstPosts.take(15)
                                moviesByCategory = postsMap.toMap()
                            }
                            
                            // Load remaining categories in background without blocking UI
                            for (cat in catalogsToFetch.drop(1)) {
                                val posts = withContext(Dispatchers.IO) {
                                    runner.getPosts(provider.url, firstScraper.value, filter = cat.filter, page = 1)
                                }
                                if (posts.isNotEmpty()) {
